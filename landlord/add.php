...
try {
    if(isset($_POST['apply_property']) && isset($_POST['user_id'])){
        ...
        $property_location->addPropertyLocation($property_id,$property_number,$street,$barangay,$city,$province,$region,$latitude,$longitude,$status);
        ...
        $property_amenities->addAmenities($property_id,$aircon, $bathroom, $cabinet,$cctv,$drinking_water,$elevator,$emergency_exit,$food_hall,$laundry, $lounge, $microwave, $parking,$refrigerator,$security, $sink,$study_area, $tv,$wifi,$status);
        ...
        $available->setAvailableSlots($property_id, $time_slots, $status);
        ...
        if($request){
            echo "<script>window.location.href='index.php?apply=1';</script>";
            exit();
        } else {
