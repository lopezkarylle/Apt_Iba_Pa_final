...
class Property 
{
	...
	public function __construct($property_type, $property_name, $owner_id, $total_rooms, $total_floors, $description, $property_number, $street, $region, $province, $city, $barangay, $postal_code, $latitude, $longitude, $reservation_fee, $advance_deposit, $status)
	{
    ...
    public function addProperty(){
        try {
            //status 1=active, 2=pending, 0=inactive
			$sql = "INSERT INTO apt_properties SET property_type=?, property_name=?, owner_id=?, total_rooms=?, total_floors=?, description=?, property_number=?, street=?, region=?, province=?, city=?, barangay=?, postal_code=?, latitude=?, longitude=?, reservation_fee=?, advance_deposit=?, status=?"; 
			$statement = $this->connection->prepare($sql);
			$statement->execute([
